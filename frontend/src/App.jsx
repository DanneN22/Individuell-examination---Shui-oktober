...
export default function App() {
  ...
  const create = async (e) => {
    ...
    await api.post('/messages', { username: username.trim(), text: text.trim() })
    setUsername('')
    setText('')
    await load()
  }
  ...
  const saveEdit = async () => {
    if (!editingId) return
    await api.put(`/messages/${editingId}`, { username: username.trim(), text: text.trim() })
    setEditingId(null)
    setUsername('')
    setText('')
    await load()
      ...
      <ul style={{listStyle:'none', padding:0, display:'grid', gap:12}}>
        {messages.map(m => (
          <li key={m.id} style={{border:'1px solid #ddd', borderRadius:12, padding:12}}>
            ...
            <p style={{margin:'8px 0 0', whiteSpace:'pre-wrap'}}>{m.text}</p>
